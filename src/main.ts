...
const noOfCustomVillagerToSummon: number = 4;
...
MCFunction(
  ...
  () => {
    ...
    _.if(counter.matches([4, Infinity]), () => {
      counter.set(0);
      tag(
        Selector("@e", {
          type: "minecraft:villager",
      ...
      execute
        .as(Selector("@e", { type: "minecraft:villager", tag: "select" }))
        .at(self)
        .run(() => {
          for (let j = 0; j < noOfCustomVillagerToSummon; j++) {
            pickRandomVillager();
          }
        });
    });
...
let pickRandomVillager: MCFunctionInstance = MCFunction(
  "pick_random_villager",
  () => {
    rng.set(uniform(0, villagerNames.length));
    villagerNames.forEach((value, i) => {
      _.if(rng.matches(i), () => {
        functionCmd(`mo_villager:villagers/${value}`);
      });
    });
  }
);
